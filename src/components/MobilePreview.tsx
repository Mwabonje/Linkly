...
export function MobilePreview({ user, links, theme = 'dark' }: MobilePreviewProps) {
  ...
  return (
    <div className="w-[380px] h-[780px] relative">
      {/* Device frame */}
      <div className="absolute inset-0 border-[14px] border-border bg-background rounded-[3rem] shadow-2xl overflow-hidden ring-1 ring-white/10">
        
        {/* Notch */}
        <div className="absolute top-0 inset-x-0 h-6 flex justify-center">
          <div className="w-32 h-6 bg-border rounded-b-3xl"></div>
        </div>

        {/* Content */}
        <div className="w-full h-full pt-16 px-6 pb-8 overflow-y-auto bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-surface via-background to-background">
          {user ? (
          ...
          <div className="flex flex-col space-y-4 w-full relative z-10">
            {activeLinks.map((link) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group flex items-center justify-between w-full p-4 rounded-[20px] bg-surface/80 border border-border backdrop-blur-sm hover:bg-surface-hover transition-all font-medium text-center shadow-lg min-h-[60px]"
          ...
          <div className="mt-12 mb-6 text-center">
             <div className="flex items-center justify-center space-x-1 text-[10px] font-semibold tracking-[0.2em] text-muted uppercase">
